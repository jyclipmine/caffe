...
Note 2: If the script to install GOP dependencies does not work on early Ubuntu version (e.g. 12.04 LTS), you may add the Ubuntu 14.04 package source to your software source list. For example, if you want to use the UC Davis source, add `deb http://mirror.math.ucdavis.edu/ubuntu/ trusty main universe` to your `/etc/apt/sources.list`, run `sudo apt-get update` and run `sudo apt-get install cmake g++-4.8 libeigen3-dev libpng-dev libjpeg-dev`. Or, you can install the GOP dependencies (especially g++-4.8 and Eigen3) manually.
