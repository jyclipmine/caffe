...
DEFINE_int32(camera_id, 0,
    "The index of camera used for this demo (usually 0).");
...
int main(int argc, char** argv) {
  ...
  // general parameters
  int camera_id = FLAGS_camera_id;
  CvCapture* pCapture = cvCreateCameraCapture(camera_id);
  CHECK(pCapture) << "Cannot setup camera at index = " << camera_id << ". Please check your camera settings.";
  const int max_proposal_num = 1000;
  const int class_num = 7604;
  const int scale_num = FLAGS_scale;
  ...
  Net<float> caffe_test_net(FLAGS_model);
  ...
  // load data from disk
  load_channel_mean(channel_mean, FLAGS_channelmean.c_str());
  load_class_name(class_name_vec, FLAGS_classname.c_str());
