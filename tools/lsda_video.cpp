...
const IplImage* read_from_camera(CvCapture* pCapture) {
  IplImage* old = 0;
  for (int i = 0; i < 5; i++)
    old = cvQueryFrame(pCapture);

  IplImage* pFrame_l = cvCreateImage(cvSize(640, 480), old->depth, old->nChannels);
  IplImage* pFrame_r = cvCreateImage(cvSize(480, 640), old->depth, old->nChannels);
  cvResize(old, pFrame_l);
  cvTranspose(pFrame_l, pFrame_r);
  CHECK(pFrame_r)
