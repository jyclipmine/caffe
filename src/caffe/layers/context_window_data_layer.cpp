...
void ContextWindowDataLayer<Dtype>::InternalThreadEntry() {
  // At each iteration, sample N windows where N*p are foreground (object)
  // windows and N*(1-p) are background (non-object) windows

  Dtype* top_data_det = prefetch_data_det_.mutable_cpu_data();
  Dtype* top_data_cls = prefetch_data_cls_.mutable_cpu_data();
  ...
  // zero out batch
  caffe_set(prefetch_data_det_.count(), Dtype(0), top_data_det);
  caffe_set(prefetch_data_cls_.count(), Dtype(0), top_data_cls);
  ...
  for (int is_fg = 0; is_fg < 2; ++is_fg) {
    for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
      ...
      pair<std::string, vector<int> > image =
          image_database_[window[ContextWindowDataLayer<Dtype>::IMAGE_INDEX]];
      ...
      int x1 = window[ContextWindowDataLayer<Dtype>::X1];
      int y1 = window[ContextWindowDataLayer<Dtype>::Y1];
      int x2 = window[ContextWindowDataLayer<Dtype>::X2];
      int y2 = window[ContextWindowDataLayer<Dtype>::Y2];
      ...
      cv::resize(cv_resized_img, cv_resized_img,
          cv_crop_size, 0, 0, cv::INTER_LINEAR);
      ...
      for (int c = 0; c < channels; ++c) {
        for (int h = 0; h < cv_cropped_img.rows; ++h) {
          for (int w = 0; w < cv_cropped_img.cols; ++w) {
            ...
            top_data_det[((item_id * channels + c) * crop_size + h + pad_h)
                     * crop_size + w + pad_w]
                = (pixel
      ...
      for (int c = 0; c < channels; ++c) {
        for (int h = 0; h < cv_resized_img.rows; ++h) {
          for (int w = 0; w < cv_resized_img.cols; ++w) {
            Dtype pixel =
                static_cast<Dtype>(cv_resized_img.at<cv::Vec3b>(h, w)[c]);

            top_data_cls[((item_id * channels + c) * crop_size + h + pad_h)
                     * crop_size + w + pad_w]
                = (pixel
                    - mean[(c * mean_height + h + mean_off + pad_h)
                           * mean_width + w + mean_off + pad_w])
      ...
      // get window label
      top_label[item_id] = window[ContextWindowDataLayer<Dtype>::LABEL];
      ...
      inf << image.first << std::endl
          << window[ContextWindowDataLayer<Dtype>::X1]+1 << std::endl
          << window[ContextWindowDataLayer<Dtype>::Y1]+1 << std::endl
          << window[ContextWindowDataLayer<Dtype>::X2]+1 << std::endl
          << window[ContextWindowDataLayer<Dtype>::Y2]+1 << std::endl
          << do_mirror << std::endl
...
template <typename Dtype>
ContextWindowDataLayer<Dtype>::~ContextWindowDataLayer<Dtype>() {
  JoinPrefetchThread();
}

template <typename Dtype>
void ContextWindowDataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  do {
    ...
    for (int i = 0; i < num_windows; ++i) {
      ...
      vector<float> window(ContextWindowDataLayer::NUM);
      window[ContextWindowDataLayer::IMAGE_INDEX] = image_index;
      window[ContextWindowDataLayer::LABEL] = label;
      window[ContextWindowDataLayer::OVERLAP] = overlap;
      window[ContextWindowDataLayer::X1] = x1;
      window[ContextWindowDataLayer::Y1] = y1;
      window[ContextWindowDataLayer::X2] = x2;
      window[ContextWindowDataLayer::Y2] = y2;

      // add window to foreground list or background list
      if (overlap >= fg_threshold) {
        int label = window[ContextWindowDataLayer::LABEL];
        CHECK_GT(label, 0);
        ...
        window[ContextWindowDataLayer::LABEL] = 0;
        window[ContextWindowDataLayer::OVERLAP] = 0;
  ...
  prefetch_data_det_.Reshape(batch_size, channels, crop_size, crop_size);
  prefetch_data_cls_.Reshape(batch_size, channels, crop_size, crop_size);

  LOG(INFO) << "output data det size: " << (*top)[0]->num() << ","
      << (*top)[0]->channels() << "," << (*top)[0]->height() << ","
      << (*top)[0]->width();
  LOG(INFO) << "output data cls size: " << (*top)[1]->num() << ","
  ...
  prefetch_data_det_.mutable_cpu_data();
  prefetch_data_cls_.mutable_cpu_data();
...
template <typename Dtype>
void ContextWindowDataLayer<Dtype>::CreatePrefetchThread() {
...
template <typename Dtype>
void ContextWindowDataLayer<Dtype>::JoinPrefetchThread() {
  CHECK(!WaitForInternalThreadToExit()) << "Pthread joining failed.";
}

template <typename Dtype>
unsigned int ContextWindowDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
void ContextWindowDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  // First, join the thread
  JoinPrefetchThread();
  // Copy the data
  caffe_copy(prefetch_data_det_.count(), prefetch_data_det_.cpu_data(),
             (*top)[0]->mutable_cpu_data());
  caffe_copy(prefetch_data_cls_.count(), prefetch_data_cls_.cpu_data(),
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(ContextWindowDataLayer, Forward);
#endif

INSTANTIATE_CLASS(ContextWindowDataLayer);
